...
#ifndef PLATFORM_FREEBSD
/* compress 512-bits */
static int sha256_compress(struct sha256_state_n *md, unsigned char *buf)
{
	u32 S[8], W[64], t0, t1;
	u32 t;
	int i;
...
static void sha256_init(struct sha256_state_n *md)
{
	md->curlen = 0;
	md->length = 0;
...
*/
static int sha256_process(struct sha256_state_n *md, unsigned char *in,
			  unsigned long inlen)
{
	unsigned long n;
...
/**
   ...
   @return CRYPT_OK if successful
*/
static int sha256_done(struct sha256_state_n *md, unsigned char *out)
{
	int i;
